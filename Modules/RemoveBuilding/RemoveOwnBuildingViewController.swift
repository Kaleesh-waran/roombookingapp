//
//  RemoveOwnBuildingViewController.swift
//  RoomBookingApp
//
...
protocol RemoveBuildingDelegate: AnyObject {
    
    func didRemoveBuilding()
}

protocol ViewToPresenterRemoveOwnBuildingProtocol {
    
    var view: PresenterToViewRemoveOwnBuildingProtocol? { get set }
    var interactor: PresenterToInteractorRemoveOwnBuildingProtocol? { get set }
    var router: PresenterToRouterRemoveOwnBuildingProtocol? { get set }
...
class RemoveOwnBuildingViewController: UIViewController,UIPickerViewDelegate,UIPickerViewDataSource {
    
    // MARK: - Properties
    var presenter: ViewToPresenterRemoveOwnBuildingProtocol?
    ...
    func setUp(){
        ...
        let presenter  = RemoveOwnBuildingPresenter()
        let interactor = RemoveOwnBuildingInteractor()
        let router     = RemoveOwnBuildingRouter()
    ...
    private func setUpBuildingComponent(){
        ...
        NSLayoutConstraint.activate([
            ...
            buildingTextLabel.heightAnchor.constraint(equalToConstant: 35)
        ])
    ...
    @objc func addButtonTapped(){
        ...
        let floorNumber = Int(floorTextField.text ?? "")
        let roomNumber = Int(roomTextField.text ?? "")
        
        CoreDataManager.shared.removeDataFromCoreData(buildingName: buildingName, floorNumber: floorNumber, roomNumber: roomNumber)
            
        self.dismiss(animated: false) {
            
            self.delegate?.didRemoveBuilding()
...
extension RemoveOwnBuildingViewController: PresenterToViewRemoveOwnBuildingProtocol{
